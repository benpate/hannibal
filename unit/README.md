...
### Usage
```go
value := unit.NewValue("http://foo.com")
...
## Interfaces
Everything implements the `unit.Value` interface, which provides several low-level manipulations for reading, writing, and transforming values.  You can implement this interface in other packages to use other custom types with the rest of the hannibal library.

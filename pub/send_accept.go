...
func SendAcceptQueueTask(actor Actor, acceptID string, activity streams.Document) QueueTask {
	return NewQueueTask(func() error {
		return SendAccept(actor, acceptID, activity)
	})
}
...
func SendAccept(actor Actor, acceptID string, activity streams.Document) error {
